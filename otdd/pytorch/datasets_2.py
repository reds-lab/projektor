...
from .utils_2 import interleave, process_device_arg, random_index_split, \
...
def load_torchvision_data(dataname, valid_size=0.1, splits=None, shuffle=True,
                    ...
                    data=None, datadir=None, download=True, filt=False, print_stats = False, tar_class=None):
    ...
    #Get only the data with target label
    if tar_class is not None:
        tar_label_train = list(np.where(np.array(train.targets) == tar_class)[0])
        train = Subset(train,tar_label_train)
        train.targets = [tar_class for i in range(len(tar_label_train))]
        train.indices = np.arange(len(tar_label_train))
        train.classes = [i for i in range(len(train.dataset.classes))]
        tar_label_test = list(np.where(np.array(test.targets) == tar_class)[0])
        test = Subset(test,tar_label_test)
        test.targets = [tar_class for i in range(len(tar_label_test))]
        test.indices = np.arange(len(tar_label_test))
        test.classes = [i for i in range(len(test.dataset.classes))]
