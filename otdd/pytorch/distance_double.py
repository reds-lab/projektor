...
class DatasetDistance2():
    ...
    def __init__(self, D1=None, D2=None,
        ...
        if self.D1 is not None and self.D2 is not None:
            ...
            logger.warning('DatasetDistance2 initialized with empty data')
    ...
    def copy(self, keep=[], drop=[]):
        ...
        initattrs = list(inspect.signature(DatasetDistance2).parameters.keys())
        ...
        dobj = DatasetDistance2(**{k:self.__dict__[k] for k in kept_init_attrs})
...
#                     print("C Shape: ", C.shape)
                    if self.coupling_method == 'geomloss':
                        ...
                        # What is u and v from loss?
                        # loss here is Sample Loss from Geom Loss check:
#                         print("loss: ", loss(Z1, Z2))
                        u, v = loss(Z1, Z2)
#                         print("uuuu: ", u)
...
#                         print("v Shape: ", v.shape)
                        ...
                        F_i, G_j = loss(Z1, Z2)
                        π = [F_i, G_j]
                        ## Entreg is 0.1 : 1/0.1 = 10, then 10 * (  )
            #             π = torch.exp(1 / self.entreg * (u.t() + v - C))  # * (pq)

#                         print("pieeee: ", π)
#                         print("π Shape: ", π.shape)
                    elif self.coupling_method == 'pot':
                        C = C.cpu()
                        C = np.asarray(C).astype('float64')
                        # arta = np.asarray(ot.unif(Z1.shape[0])).astype('float64')
                        # artb = np.asarray(ot.unif(Z2.shape[0])).astype('float64')
                        # artM = C / C.max()
                        π = ot.sinkhorn(ot.unif(Z1.shape[0]), ot.unif(Z2.shape[0]), C / C.max()
                                        , self.entreg, numItermax=50,
        ...
        first_dist = []
        if return_coupling:
            return dist, π, W, ot_dists
        else:
            return dist, ot_dists
...
class IncomparableDatasetDistance(DatasetDistance2):
